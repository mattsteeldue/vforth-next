...
\ We want to multiply two fixed-point values with 12 bits of integer part and 4 fractional (10:6)
\ Before computing x*y, add $8000 to both factors then multiply them as unsigned.
\ That is: (x + $8000)*(y + $8000) = x*y + (x+y)*$8000 + $40000000.
\ Multiplying two (10:6) numbers yelds a (20:12) that needs a final >>6 to scale to get 10:6 again
\ and all this happens MOD $10000, then the big $40000000 can be ignored.
\ So we have to subtract a (x+y)$8000 from the result before the final >>6.
\ ... or a (x+y)$0200 after the final >>6 scaling.
\ this is 
...
\ scale HLDE from 20:12 to 10:6
    ...
    ld      h'|  a|
\
\ final subtract (x+y) * $0200  by shifting 9 places.
    pop     de|
    ldn     b'|  9 N,
    bslade,b
    sbchl   de|
